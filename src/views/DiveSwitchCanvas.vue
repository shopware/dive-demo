...
const loadFile = async (file: File, index: number) => {
  const targetCanvas = canvases[index]?.value ?? null;
  const targetQuickView = quickView.value;

  if (!targetQuickView || isSwitchingCanvas.value) {
    return;
  }

  if (targetCanvas && activeCanvas.value !== index) {
    await switchCanvasTo(index);
  }
        ...
        >
            <template #panel-0>
                <CanvasFileDropOverlay
                    class="canvasWrapper canvasWrapper0"
                    @loading="(file) => loadFile(file, 0)"
                >
                    ...
                    <button
                        :disabled="!canUseCanvas(0)"
                        @click="switchCanvasTo(0)"
                    >
                        Use this
                    </button>
                </CanvasFileDropOverlay>
            ...
            <template #panel-1>
                <CanvasFileDropOverlay
                    class="canvasWrapper canvasWrapper1"
                    @loading="(file) => loadFile(file, 1)"
                >
                    ...
                    <button
                        :disabled="!canUseCanvas(1)"
                        @click="switchCanvasTo(1)"
                    >
                        Use this
                    </button>
                </CanvasFileDropOverlay>
            ...
            <template #panel-2>
                <CanvasFileDropOverlay
                    class="canvasWrapper canvasWrapper2"
                    @loading="(file) => loadFile(file, 2)"
                >
                    ...
                    <button
                        :disabled="!canUseCanvas(2)"
                        @click="switchCanvasTo(2)"
                    >
                        Use this
                    </button>
                </CanvasFileDropOverlay>
